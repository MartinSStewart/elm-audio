function startAudio(app)
{
    ...
    if (window.AudioContext) {
        ...
        // Generate white noise buffer. Original code found here https://noisehack.com/generate-noise-web-audio-api/
        let noiseBufferSize = 2 * context.sampleRate;
        let whiteNoiseBuffer = context.createBuffer(1, noiseBufferSize, context.sampleRate);
        {
            let output = whiteNoiseBuffer.getChannelData(0);
            for (let i = 0; i < noiseBufferSize; i++) {
                output[i] = Math.random() * 2 - 1;
            }
        }

        // Generate pink noise buffer. Original code found here https://noisehack.com/generate-noise-web-audio-api/
        let pinkNoiseBuffer = context.createBuffer(1, noiseBufferSize, context.sampleRate);
        {
            let output = pinkNoiseBuffer.getChannelData(0);
        ...
        // Generate brown noise buffer. Original code found here https://noisehack.com/generate-noise-web-audio-api/
        let brownNoiseBuffer = context.createBuffer(1, noiseBufferSize, context.sampleRate);
        {
            let output = brownNoiseBuffer.getChannelData(0);
            let lastOut = 0.0;
            for (let i = 0; i < noiseBufferSize; i++) {
                let white = Math.random() * 2 - 1;
        ...
        app.ports.audioPortToJS.subscribe( ( message ) => {
            ...
            for (let i = 0; i < message.audio.length; i++) {
                ...
                switch (audio.action)
                {
                    ...
                    case "startOscillator":
                    {
                        let oscillatorType = audio.oscillatorType;
                        let frequency = audio.frequency;
                        console.log(frequency);
                        let volume = audio.volume;
                        let volumeTimelines = audio.volumeTimelines;
                        ...
                        switch (oscillatorType) {
                            ...
                            default:
                            {
                                ...
                                source.frequency.setValueAtTime(frequency,0);
                                break;
                            }
                        }
                        ...
                        source.start(posixToContextTime(startTime, currentTime));

                        let nodes = { sourceNode: source, gainNode: gainNode, volumeAtGainNodes: timelineGainNodes };
